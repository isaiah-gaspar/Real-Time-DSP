...
d_temp      .int (________)    ; Temporary variables (Re, Im) insert the number of temp variables needed
d_L         .int
d_N         ____ 		; insert the directive needed		
...
_fft:
    ...
    mov  mmap(ST1_55),_____       ; Save ST1,ST3 to AR2 and AR3
    mov  ________,AR3   
    mov  AR2,fft.d_ST1
    mov  AR3,fft.d_ST3
...
do_scale
    ...
    mov  _____,fft.d_T2            ; Save T2                     
||  mov  _____,AC0
    mov  AC0,fft.d_L            ; Initialize L=1
___ sfts AC0,T0                 ; T0=EXP
    mov  AC0,fft.d_N            ; N=1<<EXP
    mov  XAR1,XCDP              ; comment
    mov  XSP,XAR4	
    add  #___________,AR4        ; AR4 = pointer to temp
...
||  mov  ____,AC0                 ;       arranged in re,im pairs
...
||  sfts AC0,#-1                              
    ...
    mov  mmap(AC0L),_____        ;   BRC0=LE1-1
    sub  #1,AC1                 ; Init inner loop counter
    mov  mmap(AC1L),_____        ;   BRC1=(N>>L)-1   
    add  AR1,AR0   				; comment
    mov  #0,____                  ; j=0 
...
||  rptblocal inner_loop-1      ; for(i=j; i<N; i+=LE) 
    ___  *AR5+,*____,AC0        ; AC0=(X[id].re*U.re
::  mpy  *AR2-,*____,AC1        ;     -X[id].im*U.im)/SCALE	 
    masr *AR5-,*CDP-,___        ; AC1=(X[id].im*U.re                       
::  macr *AR2+,*CDP-,___        ;     +X[id].re*U.im)/SCALE
    mov  pair(hi(AC0)),dbl(*AR4); AC0H=temp.re AC1H=temp.im 
||  mov  dbl(*AR3),AC2
    xcc  scale,TC1
||  mov  AC2___#1,dual(*AR3)     ; Scale X[i] by 1/SCALE
    mov  dbl(*AR3),AC2  
scale
    add  T0,AR2
